...
**完全没编程/金融基础？** 不用怕，打开 [思考与学习/00_写给零基础的你/](./思考与学习/00_写给零基础的你/00_读我.md) 这一章，**8 篇文档手把手把你从 0 拉到"跑过回测、看懂报告"**，最快 30 分钟出第一张回测净值曲线。

**已有 Python + pandas 基础？** 直接看下面"快速开始", 3 条命令跑通。
...
│   ├── 42_质量因子分组回测.py
│   └── 43_技术与基本面多因子.py
├── 思考与学习/                # 详细中文学习文档（00 写给零基础的你 + 9 章主线）
...
核心理念：**组合优化是"放大器"不是"创造者"**（无 alpha 时救不了）；**μ 估不准是最大命门**（裸用均值方差极危险）；**等权是难打的基准**（复杂方法没明显赢就别用）；**协方差比期望收益好估太多**（min_var / risk_parity 更工程化）。

## 基本面数据模块

`src/data/fundamentals.py` + `src/factors/fundamental.py` 提供基本面数据接入 + 四类基本面因子，配套 7 篇中文文档（`思考与学习/09_基本面数据/`）。
...
核心理念：**报告期 ≠ 披露日**（基本面回测最容易爆雷的地方）；**披露日 = 报告期 + 约 45 天**（A 股监管 deadline 近似）；**价值 / 质量 / 成长 / 安全 四类因子互补**（QVGS 是巴菲特选股法的量化版）；**技术 + 基本面相关性低**，合成几乎是免费午餐；**ic_weighted 比 equal 更鲁棒**（自动识别因子方向）。
