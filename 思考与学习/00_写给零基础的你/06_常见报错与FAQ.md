...
**原因**: Windows 默认禁止跑 .ps1 脚本（包括激活虚拟环境的脚本）。

**对策**: 用**管理员身份**打开 PowerShell, 跑:
```powershell
...
**原因**: 你打开了一个新终端但没激活 venv。

**对策**: 看命令行最前面有没有 `(.venv)` 标识。 没有就重新激活:
```bash
# Windows
...
**原因**: Windows 命令行默认编码是 GBK, 我们脚本里有中文输出, 直接打就乱码。
...
**永久解决** (推荐): 改 Windows 控制面板 → 区域 → 管理 → 更改系统区域设置 → 勾选 "Beta: 使用 Unicode UTF-8" → 重启。
此后命令行原生支持中文。
...
**原因**: pandas 版本不匹配。 本框架要求 pandas >= 2.0, 你装的可能太老。
...
**原因 1**: 第一次跑会编译 numpy 的 C 后端, 卡 5~10 秒正常。
**原因 2**: 如果超过 30 秒还没动静, Ctrl+C 中止, 检查代码有没有改坏（特别是改了循环条件）。
...
**原因**: git 不会自动走代理, 但你的网络需要代理才能访问 github。
...
```
**注意**: 这样配了之后, 推国内 git 服务时记得 unset, 否则会反过来报错。
...
**小白阶段建议**: 不要纠结真实数据问题, 全程跑 `examples/00`（模拟数据）就行, 等以后再处理。
...
**A**: 那是**模拟数据上的回测结果**, 不是真钱。 模拟数据是程序随机生成的, 不存在真实赚钱可能。
...
### Q: 跑出来的图在哪?
**A**: 项目根目录的 `reports/` 文件夹下, 每个策略一个子目录。

### Q: 我能跑实盘吗?
**A**: **不能**, 本框架是**纯回测**框架, 不带下单接口。 实盘要接券商 API（如 vn.py、QMT、QuickTrade）, 那是另一套系统。
...
**A**: 见 [07 完全没编程基础怎么办](./07_完全没编程基础怎么办.md), 用不着精通, 能看懂示例就行。

### Q: 我看不懂代码里的 `from __future__ import annotations`, 是不是没学过这玩意?
**A**: 不影响理解, **可以无视**。 它是 Python 类型提示的兼容性写法, 不改变功能。

### Q: 文档里出现的 `MultiIndex` / `corrwith` / `groupby` 是啥?
**A**: 都是 pandas 的概念。 第一次遇到不需要立刻搞懂, 跟着文档说的"做什么"就行, 用多了自然懂。 急着学就去 pandas 官方文档 quickstart。
...
按这个顺序找:
1. **本篇 FAQ** — 你正在看
2. **[05 章 / 数据源问题排查](../05_扩展指南/数据源问题排查.md)** — 数据相关
