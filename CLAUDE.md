...
- `src/strategies/` 双均线 / 横截面动量 / 价值因子
- `src/factors/` 技术因子库
- `src/ml/` 机器学习（特征工程 / 防泄露时序划分 / 模型封装 / IC评估 / ML策略 / walk-forward 滚动重训）
- `examples/` 示例（00 模拟数据全策略；01-03 真实数据策略；10-13 机器学习）
- `tests/` 单元测试
- `思考与学习/` 6 章中文学习文档（01架构 / 02术语 / 03因子策略 / 04指标 / 05扩展 / 06机器学习）
...
- 跑测试：`python tests/test_metrics.py && python tests/test_ml.py && python tests/test_walkforward.py`
...
## 下一步候选扩展（按价值排序）
1. **滚动调参**：walk-forward 每轮在训练集内用 `purged_kfold` 选超参（文档 06/08 已铺垫）。
2. **接入基本面数据**：扩展数据层支持 PE/PB/ROE，做真正的价值/质量因子。
3. **多因子合成策略**：IC 加权 / z-score 合成（文档 05 扩展指南有模板）。
4. **因子 IC 测试工具**：新建 `src/factors/evaluation.py`（文档已留接口设想）。
5. **组合优化**：从"等权选 top N"升级到均值-方差 / 风险预算。
6. **LightGBM/XGBoost**：装上后在 `src/ml/models.py:make_model` 加分支。
